...
BallTreeDensity::BallTreeDensity(const mxArray* structure) : BallTree(structure) {
  ...
  //type = static_cast<BallTreeDensity::KernelType> (mxGetScalar(mxGetField(structure,0,"type"))); 
  
  double passedValue = mxGetScalar(mxGetField(structure,0,"type"));
  type = static_cast<BallTreeDensity::KernelType> ((int)passedValue); 
