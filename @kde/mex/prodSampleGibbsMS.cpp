...
void initIndices(void) 
{
  ...
  for (j=0; j<(unsigned int)Ndens; j++) {
...
{
  for (unsigned int j=0; j<(unsigned int)Ndens; j++) {
    for (unsigned int z=0; z<(unsigned int)Ndim; z++) {
...
{
  for (unsigned int j=0; j<(unsigned int)Ndim; j++) {
    double mn=0, vn=0;
    for (unsigned int z=0; z<(unsigned int)Ndens; z++){ // Compute mean and variances of
...
void sampleIndex(unsigned int j) 
{
  ...
  for (i=0; i<(unsigned int)Ndim; i++) {
    ...
    for (unsigned int k=0; k<(unsigned int)Ndens; k++) {
            ...
            double *_randU, double* _randN)
{
  ...
  for (unsigned int j=0; j<(unsigned int)Ndens; j++)  // compute Max Np over all densities
  ...
  Nlevels = (unsigned int) (log((double)maxNp)/log(2.0))+1;          // how many levels to a balanced binary tree?
...
//  for (j=0;j<Ndens;j++)  printf("%d ",dNpts[j]);
    ...
    for (l=0;l<(unsigned int)Nlevels;l++) {
    ...
    for (unsigned int j=0; j<(unsigned int)Ndens; j++)              // save and
  ...
  for (j=0;j<(unsigned int)Ndens;j++) { delete[] levelList[j];  delete[] levelListNew[j]; }
